...
function getPrimaryTask() {
  return DATA.tasks.find(t => t.status === 'In Progress')
...
function getSystemState() {
  ...
  if (blockers > 0 && active > 0) return 'Moving';
  if (active === 0) return 'Idle';
  return 'Stable';
}

function getMomentum(xp, target) {
  const ratio = target ? xp / target : 0;
  if (ratio < 0.20) return 'BOOTSTRAP';
  if (ratio < 0.50) return 'EARLY MOMENTUM';
  if (ratio < 0.80) return 'STABLE BUILD';
...
function renderDashboard() {
  ...
  const momentum = getMomentum(s.xpEarned, DATA.phases[0]?.xpTarget || s.xpTarget);

  /* command strip */
  const cmdPhase = document.getElementById('cmd-phase');
  const cmdPrimary = document.getElementById('cmd-primary');
  const cmdState = document.getElementById('cmd-state');

  if (cmdPhase) cmdPhase.textContent = s.currentPhase;
  if (cmdPrimary) cmdPrimary.textContent = primaryTask ? primaryTask.name : 'No active task';
  if (cmdState) cmdState.textContent = systemState;
  ...
  /* primary action card */
  const primaryCard = document.getElementById('primary-action-card');
  if (primaryCard && primaryTask) {
    primaryCard.innerHTML = `
      <div class="primary-action-top">
        <div class="primary-action-tag">Primary Action</div>
        ${statusBadge(primaryTask.status)}
      </div>
      <div class="primary-action-title">${primaryTask.name}</div>
      <div class="primary-action-sub">
        ${primaryTask.quest} · ${primaryTask.type} · ${primaryTask.xp} XP · due ${primaryTask.due || '—'}
      </div>
      <div class="primary-action-meta">
        <span class="tag tag-level">${primaryTask.id}</span>
        <span class="tag tag-type">${primaryTask.questId}</span>
        <span class="tag tag-phase">${s.currentPhase}</span>
      </div>
    `;
  }
  ...
  const blockerCountBadge = document.getElementById('blocker-count-badge');

  if (blockerCountBadge) {
    blockerCountBadge.textContent = `${DATA.blockers.length} active`;
  }

  if (blockerSummary) {
    const firstTwo = DATA.blockers.slice(0, 2);
    blockerSummary.innerHTML = firstTwo.map(b => `
      <div class="blocker-summary-card">
        <div class="blocker-summary-title">${b.title}</div>
        <div class="blocker-summary-text">${b.desc}</div>
      </div>
    `).join('');
  }

  /* xp strip */
  const phaseTarget = DATA.phases[0]?.xpTarget || s.xpTarget;
  const progressPct = Math.min(100, ((s.xpEarned / phaseTarget) * 100));
  const xpBarFill = document.getElementById('xp-bar-fill');
  const xpBarNums = document.getElementById('xp-bar-nums');
  const momentumLabel = document.getElementById('xp-momentum-label');

  if (xpBarFill) xpBarFill.style.width = progressPct.toFixed(1) + '%';
  if (xpBarNums) xpBarNums.textContent = `${s.xpEarned} / ${phaseTarget} XP`;
  if (momentumLabel) momentumLabel.textContent = momentum;
  ...
  if (focusList) {
    const todayTasks = DATA.tasks.filter(t => t.status !== 'Done').slice(0, 4);
    ...
    actionsEl.innerHTML = pending.map((t, i) => `
      <div class="action-item">
        ...
        <div class="action-detail">
          ...
          <div class="action-sub">${t.quest} · ${t.type} · ${t.xp} XP · due ${t.due || '—'}</div>
        </div>
